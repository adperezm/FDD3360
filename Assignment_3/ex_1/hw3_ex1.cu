...
{
    /////////////////////////////////////////////////////////////
    // To do: write the GPU the version of gray scale filter   //
    /////////////////////////////////////////////////////////////
 
    //We are using one thread per pixel
    //index in x = w
...
__device__ float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    ...
    //                                                          //
    // Since each pixel applies the filter. It is the same as CPU/
    float pixel = 0.0f;
    ...
    // adperez: The only reason I can think about, is that we   //
    // need to specify that this function is called in the GPU. //
    //////////////////////////////////////////////////////////////
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        //This way, the elements in this block will be stored at the center of the shared array. Find a way to copy the neighbours
        int offset_sh = (threadIdx.y)*BLOCK_SIZE_SH + (threadIdx.x);

        //Neighbouring values to the right
        int offset_t_r1 = index_y * width + (index_x+1);
        int offset_t_r2 = index_y * width + (index_x+2);
        int offset_sh_r1 = (threadIdx.y)*BLOCK_SIZE_SH + (threadIdx.x+1);
        int offset_sh_r2 = (threadIdx.y)*BLOCK_SIZE_SH + (threadIdx.x+2);

        //Neighbouring values to the bottom
        int offset_t_d1 = (index_y+1) * width + (index_x+0);
        int offset_t_d2 = (index_y+2) * width + (index_x+0);
        int offset_sh_d1 = (threadIdx.y+1)*BLOCK_SIZE_SH + (threadIdx.x+0);
        int offset_sh_d2 = (threadIdx.y+2)*BLOCK_SIZE_SH + (threadIdx.x+0);

        //Neighbouring values diagonally
        int offset_t_di11 = (index_y+1) * width + (index_x+1);
        int offset_t_di12 = (index_y+1) * width + (index_x+2);
        int offset_sh_di11= (threadIdx.y+1)*BLOCK_SIZE_SH + (threadIdx.x+1);
        int offset_sh_di12= (threadIdx.y+1)*BLOCK_SIZE_SH + (threadIdx.x+2);
        int offset_t_di21 = (index_y+2) * width + (index_x+1);
        int offset_t_di22 = (index_y+2) * width + (index_x+2);
        int offset_sh_di21= (threadIdx.y+2)*BLOCK_SIZE_SH + (threadIdx.x+1);
        int offset_sh_di22= (threadIdx.y+2)*BLOCK_SIZE_SH + (threadIdx.x+2);
       
        /* 
        //Initialize all the entries to zero in all blocks, trhead 0
        if(threadIdx.x == 0 && threadIdx.y==0){
        	for (int i=0;i<BLOCK_SIZE_SH*BLOCK_SIZE_SH;i++){
                sh_block[i]=0;
		}
        }
        __syncthreads(); //Synchronize to ensure all is good.
        */         
       
        // Copy the values from the images into the shared array
        sh_block[offset_sh] = image[offset_t];

        int bound=(BLOCK_SIZE-1);
        if (threadIdx.x==bound){
            sh_block[offset_sh_r1]=image[offset_t_r1];
            sh_block[offset_sh_r2]=image[offset_t_r2];
        }
        if (threadIdx.y==bound){
            sh_block[offset_sh_d1]=image[offset_t_d1];
            sh_block[offset_sh_d2]=image[offset_t_d2];
        }
        if (threadIdx.x==bound && threadIdx.y==bound){
            sh_block[offset_sh_di11]=image[offset_t_di11];
            sh_block[offset_sh_di12]=image[offset_t_di12];
            sh_block[offset_sh_di21]=image[offset_t_di21];
            sh_block[offset_sh_di22]=image[offset_t_di22];
        }
        __syncthreads();

        /*
        //Print just to see all is good
        if(index_x == 100 && index_y==100){
        	for (int i=0;i<BLOCK_SIZE_SH*BLOCK_SIZE_SH;i++){
                printf("Entrys %d is: %f\n", i, sh_block[i]);
		}
        }
        __syncthreads();
        */
...
{
    BMPImage bitmap          = { 0 }; // {0} to put zeros in all entries
    float    *d_bitmap       = { 0 };
    float    *image_out[2]   = { 0 };
